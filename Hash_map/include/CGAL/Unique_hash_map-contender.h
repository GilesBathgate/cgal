...
#ifndef CGAL_UNIQUE_HASH_MAPC_H
#define CGAL_UNIQUE_HASH_MAPC_H
...
#include <CGAL/Tools/chained_map-contender.h>
#include <cstddef>
...
class Unique_hash_map_contender {
public:
    typedef Key_                                     Key;
    typedef Data_                                    Data;
    ...
    typedef Unique_hash_map_contender<Key,Data,Hash_function,Allocator> Self;

private:
    typedef contender::chained_map<Data, Allocator>   Map;
...
public:
    ...
    Unique_hash_map_contender() { m_map.xdef() = Data(); }

    Unique_hash_map_contender( const Data& deflt, std::size_t table_size = Map::min_size)
        : m_map( table_size) { m_map.xdef() = deflt; }

    Unique_hash_map_contender( const Data& deflt,
                     std::size_t table_size,
                     const Hash_function& fct)
        : m_hash_function(fct), m_map( table_size) { m_map.xdef() = deflt; }

    Unique_hash_map_contender( Key first1, Key beyond1, Data first2) {
        m_map.xdef() = Data();
...
namespace boost {
  ...
  class associative_property_map<CGAL::Unique_hash_map_contender<KeyType, ValueType,
                                                       HashFunction, Allocator> >
  {
    typedef CGAL::Unique_hash_map_contender<KeyType, ValueType, HashFunction, Allocator> C;
  ...
  template <typename KeyType, typename ValueType,
            typename HashFunction, typename Allocator>
  associative_property_map<CGAL::Unique_hash_map_contender<KeyType, ValueType,
                                                 HashFunction, Allocator> >
  make_assoc_property_map(CGAL::Unique_hash_map_contender<KeyType, ValueType,
                                                HashFunction, Allocator>& c)
  {
    return associative_property_map<CGAL::Unique_hash_map_contender<KeyType, ValueType,
                                                          HashFunction, Allocator> >(c);
  }
